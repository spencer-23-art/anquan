  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={[styles.page, { backgroundColor: colors.bg }]}>
      <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
        ...
        <TextInput style={[styles.input, { borderColor: colors.border, color: colors.text, backgroundColor: colors.cardSoft }]} placeholder="登录账号，可填写中文姓名" placeholderTextColor={colors.subtext} value={username} onChangeText={setUsername} multiline numberOfLines={1} scrollEnabled={false} keyboardType="default" autoCapitalize="none" autoCorrect={false} autoComplete="off" importantForAutofill="no" textContentType="none" />
        <TextInput style={[styles.input, { borderColor: colors.border, color: colors.text, backgroundColor: colors.cardSoft }]} placeholder="真实姓名，可填写中文" placeholderTextColor={colors.subtext} value={realName} onChangeText={setRealName} multiline numberOfLines={1} scrollEnabled={false} keyboardType="default" autoComplete="off" importantForAutofill="no" textContentType="none" />
