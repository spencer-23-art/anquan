...
def generate_description(
    ...
    user_prompt = (
        ...
        "6. 第二段先写建筑行业安全通用规范，再结合本次问题补充更具体的专业规范或管理规定。\n"
        "7. 两段都要写成完整句，不允许只写标题、日期、短语或空泛套话。\n"
        "8. 行文要自然、正式，避免每次都使用完全相同的套话。\n"
        "9. 第一段和第二段都应尽量写得充实，适合直接用于罚款通知单。\n\n"
        f"项目名称：{project_name}\n"
        f"受罚班组：{team_name}\n"
        f"违规部位：{location}\n"
        f"发现日期：{fact_date}\n"
...
def _resolve_rule_reference(user_input: str, ticket_type: FineTicketType) -> str:
    ...
    if ticket_type == FineTicketType.QUALITY:
        if any(keyword in text for keyword in ["防水", "渗漏", "漏水"]):
            return f"{base}、《建筑与市政工程防水通用规范》GB 55030-2022"
        return base

    if any(keyword in text for keyword in ["培训证", "培训证过期", "操作证", "特种作业证", "上岗证", "证书过期", "持证", "证件过期", "无证上岗", "特种作业"]):
        return f"{base}、《建筑施工特种作业人员管理规定》及《特种作业人员安全技术培训考核管理规定》"
    if any(keyword in text for keyword in ["吊装", "起重", "信号工", "司索", "塔吊", "吊物"]):
        return f"{base}、《建筑施工起重吊装工程安全技术规范》JGJ 276-2012"
    if any(keyword in text for keyword in ["高处", "登高", "安全带", "临边", "洞口", "坠落"]):
        return f"{base}、《建筑施工高处作业安全技术规范》JGJ 80-2016"
    if any(keyword in text for keyword in ["临电", "配电箱", "漏保", "电缆", "接地", "电线"]):
        return f"{base}、《施工现场临时用电安全技术规范》JGJ 46-2005"
    if any(keyword in text for keyword in ["动火", "焊接", "切割", "明火"]):
        return f"{base}、《建设工程施工现场消防安全技术规范》GB 50720-2011"
    if any(keyword in text for keyword in ["安全帽", "防护用品", "劳保", "反光衣"]):
        return f"{base}、《建筑施工作业劳动防护用品配备及使用标准》JGJ 184-2009"
    return base
